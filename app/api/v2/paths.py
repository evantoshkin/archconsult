...
from app.db.queries import execute_traverse_search, fetch_node_names, build_child_tree_by_rsm_id
...
async def traverse_search(request: TraverseRequest) -> TraverseResponse:
    try:
        results = await execute_traverse_search(
        ...
        for finish_node, data in result_data["paths"].items():
            ...
            if path_key not in path_groups:
                ...
                path_groups[path_key] = {
                    "path": named_path,
                    "integration_example_count": 1,
                    "document_rsm_id": eotar_id,
