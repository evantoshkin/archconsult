...
def build_traverse_graph_query(graph_name: str) -> str:
    return f"""
...
async def load_graph_edges() -> list[GraphEdge]:
    logger.info("Loading graph edges for Traverse algorithm")
    
    pool = get_pool()
    query = build_traverse_graph_query(settings.AGE_GRAPH_NAME)
    timeout = settings.DB_STATEMENT_TIMEOUT_MS / 1000.0
...
def matches_node(node_id: str, filter: TraverseFilter) -> bool:
    parts = node_id.split("|")
    node = {
...
async def execute_traverse_search(
    ...
    logger.info(f"Traverse: {len(start_nodes)} start nodes, {len(finish_nodes)} finish nodes")
    ...
    logger.info(f"Traverse: {len(eotar_ids)} unique eotar_rsm_ids to process")
    ...
    logger.info(f"Traverse: found paths in {len(results)} eotar groups")
    return results
